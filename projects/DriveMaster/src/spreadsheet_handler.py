...
def write_report_to_csv(report_data, filename):
    ...
    if not report_data:
        ...
        return True # Not a failure if there's nothing to write
    try:
...
def add_dropdowns_to_sheet(filename):
    """Adds dropdowns and formatting to an Excel sheet."""
    # This is a helper function, its error handling is internal
    try:
        ...
        dv_restrict.add('J2:J1048576'); dv_action.add('K2:K1048576'); dv_role.add('L2:L1048576'); dv_principal.add('M2:M1048576')
        ws.add_data_validation(dv_restrict); ws.add_data_validation(dv_action); ws.add_data_validation(dv_role); ws.add_data_validation(dv_principal)
        ...
        wb.save(filename)
        logging.info(f"Successfully added dropdowns and formatting to {filename}")
    except Exception as e:
...
def write_report_to_excel(report_data, filename):
    ...
    try:
        df = pd.DataFrame(report_data)
        action_columns = ['Action_Type', 'New_Role', 'Type of account (for ADD)', 'Email/Domain (for ADD)']
        for col in action_columns: df[col] = ''
        column_order = ['Full Path', 'Item Name', 'Item ID', 'Role', 'Principal Type', 'Email Address', 'Owner', 'Google Drive URL', 'Root Folder ID', 'Restrict Download'] + action_columns
